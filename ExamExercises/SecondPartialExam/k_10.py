...
if __name__ == '__main__':
    ...
    idx_to_split = int(x * len(dataset))

    test_set = dataset[:idx_to_split]
    train_set = dataset[idx_to_split:]

    train_X, train_Y = split_set_x_y(train_set)
    test_X, test_Y = split_set_x_y(test_set)
    ...
    importanceS = list(classifierDT.feature_importances_)
    idx = importanceS.index(min(importanceS))

    train_X_1, train_Y_1 = removed_col_split_set_x_y(train_set, idx)
    test_X_1, test_Y_1 = removed_col_split_set_x_y(test_set, idx)
    ...
    classifierMLP.fit(scalerSS.transform(train_X_1), train_Y_1)
    accuracy1 = calculate_accuracy(classifierMLP.predict(scalerSS.transform(test_X_1)),test_Y_1)
    print(f"Tocnost so StandardScaler: {accuracy1}")

    classifierMLP.fit(scalerMMS.transform(train_X_1),train_Y_1)
    accuracy2 = calculate_accuracy(classifierMLP.predict(scalerMMS.transform(test_X_1)),test_Y_1)
