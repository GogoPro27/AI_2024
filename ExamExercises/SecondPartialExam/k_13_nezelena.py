...
if __name__ == '__main__':
    ...
    train_set_NB = class0[:int(x1 * len(class0))] + class1[:int(x1 * len(class1))] + class2[:int(x1 * len(class2))]
    ...
    train_set_DT = class0[int(x1 * len(class0)):int(x2 * len(class0))] + \
                   class1[int(x1 * len(class1)):int(x2 * len(class1))] + \
                   class2[int(x1 * len(class2)):int(x2 * len(class2))]
    ...
    train_set_RF = class0[:int(x2 * len(class0))] + class1[:int(x2 * len(class1))] + class2[:int(x2 * len(class2))]
    train_X_RF = [row[:-1] for row in train_set_RF]
    train_Y_RF = [row[-1] for row in train_set_RF]
    classifierRF.fit(train_X_RF, train_Y_RF)

    test_set = class0[int(x2 * len(class0)):] + class1[int(x2 * len(class1)):] + class2[int(x2 * len(class2)):]
    test_X = [row[:-1] for row in test_set]
    test_Y = [row[-1] for row in test_set]

    predictionsNB = classifierNB.predict(test_X)
    predictionsDT = classifierDT.predict(test_X)
    predictionsRF = classifierRF.predict(test_X)

    acc = 0
    for nb, dt, rf, ty in zip(predictionsNB, predictionsDT, predictionsRF, test_Y):
        count = 0
        if nb == ty: count += 1
        if dt == ty: count += 1
        if rf == ty: count += 1
    ...
    print(f"Tochnost: {acc / len(test_Y)}")
