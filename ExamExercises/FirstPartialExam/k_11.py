...
class Snake(Problem):
    ...
    def h(self, node):
        state = node.state
        snake, dir, apples = state
        # print("vlagam")
        return self.lowest_manhattan(snake[-1], apples)

    def lowest_manhattan(self, start, left_apples):
        if len(left_apples) == 0:
            return 0
        value = self.manhattan(start, left_apples[0])
        apple = left_apples[0]
        for a in left_apples:
            if self.manhattan(start, a) < value:
                value = self.manhattan(start, a)
                apple = a
        new_left_apples = [a for a in left_apples if a != apple]
        return value + self.lowest_manhattan(apple, new_left_apples)
